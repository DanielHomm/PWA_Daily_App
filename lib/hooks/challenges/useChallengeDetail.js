...
import { isCheckinAllowed, isPeriodCompleted } from "@/utils/challenge/frequencies";
...
export function useChallengeDetail(challengeId) {
  ...
  const groupStats = useMemo(() => {
    let totalCheckins = 0;
    let checkinsToday = 0;
    const activeMembersTodaySet = new Set();
    const activeMembersTotalSet = new Set();

    for (const c of allCheckins) {
      totalCheckins++;
      activeMembersTotalSet.add(c.user_id);

      if (toDateOnly(c.date) === todayStr) {
        checkinsToday++;
        activeMembersTodaySet.add(c.user_id);
      }
    }
    ...
    return {
      totalCheckins,
      checkinsToday,
      activeMembersToday: activeMembersTodaySet.size,
      activeMembersTotal: activeMembersTotalSet.size,
      totalMembers: members.length
    };
  }, [allCheckins, members.length, todayStr]);
  ...
  // New Helper: Check if period is completed (for weekly/monthly)
  const isTaskCompletedForPeriod = useCallback((targetDate, task) => {
    if (!task) return false;

    // Build completed dates list (same as above, maybe refactor to share list later)
    const completedDates = [];
    myCheckinsMap.forEach((subIds, dateStr) => {
      if (subIds.has(task.id)) {
        completedDates.push(dateStr);
    ...
    return isPeriodCompleted(targetDate, task.frequency, completedDates);
  }, [myCheckinsMap]);
